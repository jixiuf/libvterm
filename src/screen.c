...
typedef struct OverflowLine {
  ScreenCell *cells;
  bool continuation;
  struct OverflowLine *next;
} OverflowLine;
...
static void resize_buffer(VTermScreen *screen, int bufidx, int new_rows, int new_cols, bool active, VTermStateFields *statefields)
{
  ...
  OverflowLine *overflow_head = NULL;

  fprintf(stderr, "[LIBVTERM-RESIZE] resize_buffer START: bufidx=%d active=%d old=%dx%d new=%dx%d cursor=(%d,%d)\n",
      bufidx, active, old_cols, old_rows, new_cols, new_rows, old_cursor.col, old_cursor.row);
  ...
  while(old_row >= 0) {
    ...
    while(REFLOW && old_lineinfo && old_row >= 0 && old_lineinfo[old_row].continuation)
      old_row--;
    /* If we went past the top (row 0 had continuation), clamp to 0 */
    if(old_row < 0)
      old_row = 0;
    int old_row_start = old_row;

    /* 调试：打印 continuation 标志状态 */
    fprintf(stderr, "[LIBVTERM-RESIZE] Continuation check: row %d, continuation=%d\n",
        old_row_end, old_lineinfo ? old_lineinfo[old_row_end].continuation : -1);
    if(old_row_start != old_row_end) {
      fprintf(stderr, "[LIBVTERM-RESIZE] Found continuation chain: rows [%d..%d]\n",
          old_row_start, old_row_end);
      for(int r = old_row_start; r <= old_row_end && r < old_row_start + 10; r++) {
        fprintf(stderr, "[LIBVTERM-RESIZE]   row %d: continuation=%d doublewidth=%d doubleheight=%d\n",
            r, old_lineinfo[r].continuation, old_lineinfo[r].doublewidth, old_lineinfo[r].doubleheight);
      }
    }

    fprintf(stderr, "[LIBVTERM-RESIZE] Processing logical line: old_row_start=%d old_row_end=%d old_row=%d new_row=%d\n",
        old_row_start, old_row_end, old_row, new_row);
    /* 当窗口变宽时，检查 continuation 链的实际内容宽度
    ...
    for(int row = old_row_start; row <= old_row_end; row++) {
      if(REFLOW && row < (old_rows - 1) && old_lineinfo[row + 1].continuation)
        ...
        int row_width = line_popcount(old_buffer, row, old_rows, old_cols);
        fprintf(stderr, "[LIBVTERM-RESIZE]   row %d: popcount=%d (continuation=%d)\n", 
            row, row_width, old_lineinfo ? old_lineinfo[row].continuation : -1);
        width += row_width;
      }
    }

    fprintf(stderr, "[LIBVTERM-RESIZE] logical line rows [%d..%d] total_width=%d prepend_width=%d\n",
        old_row_start, old_row_end, width, prepend_width);
    ...
    fprintf(stderr, "[LIBVTERM-RESIZE] new_height=%d width=%d new_cols=%d REFLOW=%d final_blank_row=%d\n",
        new_height, width, new_cols, REFLOW, final_blank_row);
        ...
        (!active || new_cursor.row == -1 || (new_cursor.row - new_row_start) < new_rows))
    {
      ...
      fprintf(stderr, "[LIBVTERM-RESIZE] SCROLL: %d rows +%d downwards to fit content, final_blank_row=%d->%d\n",
          rowcount, downwards, final_blank_row, final_blank_row + downwards);
    ...
    OverflowLine *segment_head = NULL;
    OverflowLine *segment_tail = NULL;

    int prepend_col = 0;  /* Track position in prepend_cells */

    for(new_row = new_row_start, old_row = old_row_start; new_row <= new_row_end; new_row++) {
      int count = width >= new_cols ? new_cols : width;
      width -= count;

      ScreenCell *target_cells;
      if(new_row < 0)
        target_cells = vterm_allocator_malloc(screen->vt, sizeof(ScreenCell) * new_cols);
      else
        target_cells = &new_buffer[new_row * new_cols];

      int new_col = 0;

      /* First, copy from prepend_cells if available */
      while(count && prepend_cells && prepend_col < prepend_width) {
        target_cells[new_col] = prepend_cells[prepend_col];
#ifdef DEBUG_REFLOW
        fprintf(stderr, "Copying prepend '%c' to (%d,%d)\n", target_cells[new_col].chars[0], new_row, new_col);
#endif
        prepend_col++;
        new_col++;
        count--;
      ...
      /* Then copy from old_buffer */
      while(count) {
        /* TODO: This could surely be done a lot faster by memcpy()'ing the entire range */
        target_cells[new_col] = old_buffer[old_row * old_cols + old_col];
#ifdef DEBUG_REFLOW
        fprintf(stderr, "Copying '%c' to (%d,%d)\n", target_cells[new_col].chars[0], new_row, new_col);
#endif
      ...
      while(new_col < new_cols) {
        clearcell(screen, &target_cells[new_col]);
        new_col++;
      }

      bool continuation = (new_row > new_row_start);
      if(new_row < 0) {
        OverflowLine *line = vterm_allocator_malloc(screen->vt, sizeof(OverflowLine));
        line->cells = target_cells;
        line->continuation = continuation;
        line->next = NULL;
        if(segment_tail)
          segment_tail->next = line;
        else
          segment_head = line;
        segment_tail = line;
      }
    ...
    if(segment_head) {
      segment_tail->next = overflow_head;
      overflow_head = segment_head;
    }

    if(prepend_cells)
      vterm_allocator_free(screen->vt, prepend_cells);
  ...
  if(active && new_cursor.col != -1 && new_cursor.row < 0)
    new_cursor.row = 0;
  ...
  fprintf(stderr, "[LIBVTERM-RESIZE] resize_buffer END: bufidx=%d new_cursor=(%d,%d) overflow_head=%p\n",
      bufidx, new_cursor.row, new_cursor.col, (void *)overflow_head);

  while(overflow_head) {
    OverflowLine *next = overflow_head->next;

    VTermScreenCell *vterm_cells = vterm_allocator_malloc(screen->vt, sizeof(VTermScreenCell) * new_cols);
    for(int col = 0; col < new_cols; col++) {
      ScreenCell *intcell = &overflow_head->cells[col];
      VTermScreenCell *cell = &vterm_cells[col];

      for(int i = 0; i < VTERM_MAX_CHARS_PER_CELL; i++) {
        cell->chars[i] = intcell->chars[i];
        if(!intcell->chars[i]) break;
      }
      ...
      cell->width = 1;
      if(col < (new_cols - 1) && overflow_head->cells[col+1].chars[0] == (uint32_t)-1)
        cell->width = 2;
    }

    if(screen->callbacks_has_pushline4 && screen->callbacks && screen->callbacks->sb_pushline4)
      (*screen->callbacks->sb_pushline4)(new_cols, vterm_cells, overflow_head->continuation, screen->cbdata);
    else if(screen->callbacks && screen->callbacks->sb_pushline)
      (*screen->callbacks->sb_pushline)(new_cols, vterm_cells, screen->cbdata);

    vterm_allocator_free(screen->vt, vterm_cells);
    vterm_allocator_free(screen->vt, overflow_head->cells);
    vterm_allocator_free(screen->vt, overflow_head);
    overflow_head = next;
  }
  ...
  if(new_row >= 0 && bufidx == BUFIDX_PRIMARY &&
    ...
    fprintf(stderr, "[LIBVTERM-RESIZE] Processing scrollback pop: new_row=%d pop_cols=%d\n", new_row, pop_cols);
    while(new_row >= 0) {
      bool continuation = false;
      int popped = 0;
      if(screen->callbacks && screen->callbacks->sb_popline4)
      ...
      fprintf(stderr, "[LIBVTERM-RESIZE] sb_popline: popped=%d continuation=%d\n", popped, continuation);
      ...
      int needed_rows = (count + new_cols - 1) / new_cols;
      if (needed_rows == 0) needed_rows = 1;

      int start_row = new_row - needed_rows + 1;

      fprintf(stderr, "[LIBVTERM-RESIZE] scrollback chunk: width=%d needed_rows=%d start_row=%d new_row=%d\n",
          width, needed_rows, start_row, new_row);

      if(start_row < 0) {
         vterm_allocator_free(screen->vt, cells);
         break;
      }
      ...
      for(int r = start_row; r <= new_row; r++) {
          for(int c = 0; c < new_cols; c++) {
              if (chunk_col < count && chunk_col < pop_cols) {
                 ...
                 clearcell(screen, &new_buffer[r * new_cols + c]);
              }
          }
          new_lineinfo[r].continuation = (r > start_row) || (r==start_row && continuation);
      }
  ...
  if(new_row >= 0) {
    ...
    fprintf(stderr, "[LIBVTERM-RESIZE] Final scroll: moverows=%d new_row=%d new_cursor.row=%d->%d\n",
        moverows, new_row, new_cursor.row, new_cursor.row - (new_row + 1));
  ...
  fprintf(stderr, "[LIBVTERM-RESIZE] resize_buffer COMPLETE: bufidx=%d final_cursor=(%d,%d)\n",
      bufidx, statefields->pos.row, statefields->pos.col);

  /* 调试：打印每行的内容宽度 */
  fprintf(stderr, "[LIBVTERM-RESIZE] Screen content after resize (bufidx=%d):\n", bufidx);
  for(int r = 0; r < new_rows && r < 15; r++) {
    int w = line_popcount(new_buffer, r, new_rows, new_cols);
    fprintf(stderr, "[LIBVTERM-RESIZE]   row %d: width=%d continuation=%d\n",
        r, w, new_lineinfo[r].continuation);
  }
...
static int resize(int new_rows, int new_cols, VTermStateFields *fields, void *user)
{
  ...
  fprintf(stderr, "[LIBVTERM-RESIZE] screen.resize: old=%dx%d new=%dx%d altscreen_active=%d\n",
      old_cols, old_rows, new_cols, new_rows, altscreen_active);
  ...
  fprintf(stderr, "[LIBVTERM-RESIZE] screen.resize COMPLETE: new=%dx%d\n", new_cols, new_rows);
